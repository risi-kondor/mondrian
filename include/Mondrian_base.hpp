...
class CopyWarning{
public:
  CopyWarning(const _flag=true):flag(_flag){}
  CopyWarning& operator=(const bool _flag){flag=_flag;}
  bool flag=true;
};

template<class TYPE>
TYPE silent_copy(const TYPE& x){return TYPE(x,CopyWarning(false));}
*/

/*
template<class MATRIX>
class MatrixIsSymmetric{
public:
  MatrixIsSymmetric(MATRIX& _obj): obj(_obj){};
  operator MATRIX() const{return obj;}
public:
  MATRIX& obj;
};

template<class MATRIX>
class MatrixIsSymmetricTemp{
public:
  MatrixIsSymmetricTemp(MATRIX& _obj): obj(_obj){};
  operator MATRIX() const{return std::move(obj);}
public:
  MATRIX& obj;
};
...
template<class MATRIX>
MatrixIsSymmetric<MATRIX> is_symmetric(MATRIX& M){
  return MatrixIsSymmetric<MATRIX>(M);}

template<class MATRIX>
MatrixIsSymmetricTemp<MATRIX> is_symmetric(MATRIX&& M){
  return MatrixIsSymmetricTemp<MATRIX>(M);}
*/

/*
class biscalar{
public:
  biscalar(SCALAR& _first, SCALAR& _second): first(_first), second(_second){}
  operator SCALAR() const {return first;}
  biscalar& operator=(const SCALAR v){first=v; second=v; return *this;}
public:
  SCALAR& first;
  SCALAR& second;
};
